...
- **URI (Uniform Resource Identifier)**: it is the address of the requested resource, often referred to as the path or endpoint. For example, `/articles/42` refers to article number 42.
...
Request headers provide additional information about the request, such as the type of content being sent, necessary authorizations, or client information. Each header is a key-value pair.

Common headers include:

- **Host**: the server's address (e.g., `www.example.com`).
- **User-Agent**: client information, such as the type of browser being used.
...
The request body is an optional part that contains data sent to the server, for example in a `POST` or `PUT` request. This data can be in various formats (JSON, XML, form-encoded, etc.). For a `GET` request, the request body is usually absent as the data is passed in the URI (via query parameters).
...
- **GET**: Used to retrieve information from a resource. A `GET` request should not change the state of the resource.
...
Certain headers play a crucial role in processing HTTP requests:
...
- **API Path**: `/api/v1/users/create` indicates the endpoint URL.
  - **Query Parameters**: `?notify=true&admin=false` specifies that the user should be notified and not created as an administrator.
- **HTTP Headers**:
...
This request will then be processed by the backend, which will have access to all this information.

## HTTP Status Codes

The response to a request comes with a status code ranging from 100 to 527. The first digit is used to specify one of the five response categories:

- <Highlight color="#6495ED">1XX</Highlight> : informational
- <Highlight color="#00A36C">2XX</Highlight> : success
- <Highlight color="#7F00FF">3XX</Highlight> : redirection
- <Highlight color="#FFAC1C">4XX</Highlight> : client error
- <Highlight color="#FF3131">5XX</Highlight> : server error

The most common status codes are:

- <Highlight color="#00A36C">200</Highlight> : successful request
- <Highlight color="#00A36C">201</Highlight> : success with resource creation
- <Highlight color="#7F00FF">301 and 302</Highlight> : redirection, permanent and temporary respectively
- <Highlight color="#FFAC1C">401</Highlight> : user not authenticated
- <Highlight color="#FFAC1C">403</Highlight> : access denied
- <Highlight color="#FFAC1C">404</Highlight> : resource not found
- <Highlight color="#FF3131">500, 502, and 503</Highlight> : server errors

See the [complete list of HTTP status codes](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes)

## Security with HTTPS

**HTTPS** (Hypertext Transfer Protocol Secure) is the combination of HTTP with a layer of encryption called **TLS**. HTTPS allows the visitor to verify the identity of the website they are accessing, thanks to an authentication certificate issued by a trusted third-party authority.

TLS (or SSL) operates in a client-server mode. It aims to achieve the following security objectives:
